...
VALOR INTEGER;
BEGIN
GUARDAR_CALIFICACIONES(VALOR, 'PROC. DIG. SEÑALES',9);
END;
/

--VERIFICAMOS
SELECT * FROM CALIFICACIONES; -- ESTA RELACIONADO A UN CURSOR EXPLICITO
SELECT COUNT(*) FROM CALIFICACIONES; -- ESTA RELACIONADO A UN CURSOR IMPLICITO
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('CALIFICACIÓN: '||REC.VALOR || '   MATERIA: '||REC.MATERIA);
  END LOOP;
  END;
  /
